...
class library {
  ...
  addBook(addedBook: Book): void {
    this.books.push(addedBook);
  }
  removeBook(title: string): void {
    this.books = this.books.filter(
      (bookElement) => bookElement.title !== title
    );
  }

  searchBook(titlee: string): Book[] {
  ...
  borrowBook(titlee: string): boolean {
  ...
  returnBook(titlee: string): boolean {
    const book = this.books.find((bookElement) => {
...
class User {
  ...
  borrowBook1(library2: library, title: string): boolean {
